...
class TypingGameApp:
    ...
    def set_mode_selection_screen(self) -> None:
        ...
        quiz_table = ttk.Treeview(self.mainframe, columns=column, height=5)
        quiz_table.column("#0", width=0, stretch="no")
        quiz_table.column(name, anchor="w", width=200)
        quiz_table.column(description, anchor="w", width=300)
        quiz_table.heading("#0", text="")
        quiz_table.heading(name, text=name, anchor="center")
        quiz_table.heading(description, text=description, anchor="center")
        for n, d in overview2index.keys():
            wrapped_n = "\n".join(textwrap.wrap(n, width=20))
            wrapped_d = "\n".join(textwrap.wrap(d, width=20))
            quiz_table.insert(parent="", index="end", values=(wrapped_n, wrapped_d))
        quiz_table.grid(column=0, row=0, columnspan=5, padx=10, pady=20)
