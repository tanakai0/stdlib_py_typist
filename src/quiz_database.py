...
def create_database(database_folder, quiz_template):
    ...
    with sqlite3.connect(path) as conn:
        ...
        ''')
        cursor.execute("INSERT INTO overview (name, description) VALUES (?, ?)", (quiz_template.name, quiz_template.description))
...
def main():
    database_foler = Path("./assets/database")
    create_database(database_foler, TestTyping)
    create_database(database_foler, TestKanji)
    create_database(database_foler, TestKencho)
